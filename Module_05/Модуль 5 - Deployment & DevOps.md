...
<grid drag="80 40" drop="10 50">
**От кода до живого сайта в продакшене**

Фокус на практическом понимании, а не заучивании команд.
Системное мышление: Код → Build → Deploy → Production
</grid>

---

## Блок 1: Работа с терминалом
### Ubuntu: ваш новый инструмент
...
### От доменного имени до вашего кода
...
📖 **[Все детали в файле 04](04_FastAPI%20-%20от%20теории%20к%20практике.md#часть-5--превращаем-в-системную-службу-systemd)**
</grid>
...
<grid drag="100 70" drop="0 35" style="zoom: 1.4;">
```bash
# Включить службу
...
## Полный путь от кода до продакшена

<grid drag="100 80" drop="0 20" style="zoom: 1.5;">
```mermaid
graph TD
    A[Код на локальной машине] --> B{Frontend или Backend?}
    B -->|Frontend| C[npm run build]
    B -->|Backend| D[pip install зависимости]
    C --> E[Upload dist/ через Cockpit]
    D --> F[Создать .service файл]
    E --> G[Nginx раздаёт статику]
    F --> H[systemctl start myapp]
    G --> I[mysite.com]
    H --> J[mysite.com/api]
```
</grid>
...
<grid drag="80 60" drop="10 25">
1. **Build Locally, Deploy Static**: Не нагружаем сервер сборкой
2. **One Server, Many Apps**: Nginx как единая точка входа
3. **Service-First Mindset**: Backend работает 24/7 автоматически
4. **Logs Are Your Friend**: `journalctl` и `nginx logs` — ваши глаза

> Системное мышление важнее знания команд
</grid>
...
1. ✅ **Frontend**: Vite App с формой для ввода
2. ✅ **Backend**: FastAPI с вызовом LLM через OpenRouter
